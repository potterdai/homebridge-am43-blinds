...
class AM43Platform {
  ...
  configurePropertiesOnAccessory(accessory) {
    ...
    accessory.disconnectIfUninteracted = () => {
      ...
      if (
        ...
        accessory.log.info("Disconnecting AM43 due to HAP inactivity")
        accessory.am43device.disconnectAsync()
      }
    }
    ...
    accessory.scanForMissingDevices = () => {
      ...
      this.log.info("Started scan for missing devices")
    }
    ...
    accessory.updateInformation = async function () {
      ...
      this.log.info("Updating device information from poll")
  ...
  configureDeviceOnAccessory(accessory, device) {
    ...
    device.on("direction", (direction) => {
      this.log.info("Notifying of new direction (0 down, 1 up): " + direction)
    ...
    device.on("targetPosition", (position) => {
      ...
      this.log.info("Notifying of new target position: " + targetPosition)
      accessory.windowCoveringService
        .getCharacteristic(this.Characteristic.TargetPosition)
    ...
    device.on("position", (position) => {
      ...
      this.log.info("Notifying of new position: " + position)
    ...
    device.on("batteryPercentage", (percentage) => {
      this.log.info("Notifying of new battery percentage: " + percentage)
  ...
  configureInformationServiceOnAccessory(accessory) {
    ...
    service
      ...
      .onGet(() => {
        if (!accessory.am43device) {
          accessory.log.info("No device found please try again", null)
          return
        }
        return accessory.am43device.id
      })

    service
      .getCharacteristic(this.Characteristic.FirmwareRevision)
      .onGet(() => this.packageJSON.version)
  ...
  configureBatteryServiceOnAccessory(accessory) {
    ...
    service
      ...
      .onGet(async () => {
        ...
        return accessory.am43device.batteryPercentage
      })
    ...
    service
      ...
      .onGet(async () => {
        ...
        accessory.am43device.updateBatteryStatusAsync();
        return accessory.am43device.batteryPercentage <= 10
      })
  ...
  configureWindowCoveringServiceOnAccessory(accessory) {
    ...
    service
      ...
      .onGet(async () => {
        ...
        if (
          ...
          accessory.log.info("Requesting position update", accessory.am43device.isConnected)
        ...
        accessory.log.info("Reporting position: " + position)
        return position
      })
    ...
    service
      ...
      .onGet(() => {
        ...
        accessory.log.info("Reporting target position: " + targetPosition)

        return targetPosition
      })
      .onSet(async (value) => {
        if (!accessory.am43device) {
          accessory.log.info("No device found please try again")
          return
        }
        accessory.lastHAPInteraction = Date.now()

        const targetPosition = 100 - value // In AM43 Devices 100% means fully closed, but in HomeKit 100% means fully opened
        accessory.log.info("setting target position: " + targetPosition)
        try {
    ...
    service
      ...
      .onGet(() => {
        ...
        accessory.lastHAPInteraction = Date.now()
        accessory.log.info("Reporting direction: " + accessory.am43device.direction)
        return accessory.am43device.direction
      })
