...
class AM43Platform {
  ...
  configureInformationServiceOnAccessory(accessory) {
    ...
    service
      ...
      .onGet(() => {
        if (!accessory.am43device) {
          accessory.log.debug("No device found please try again", null)
          return
        }
  ...
  configureWindowCoveringServiceOnAccessory(accessory) {
    ...
    service
      ...
      .onSet(async (value) => {
        if (!accessory.am43device) {
          accessory.log.debug("No device found please try again")
          return
        }
        accessory.lastHAPInteraction = Date.now()
        ...
        try {
          ...
          this.pollCurrentPosition(accessory)
        } catch (error) {
          accessory.log.debug(error)
        }
      })
  ...
  pollCurrentPosition(accessory, counter = 0) {
    if (accessory.pollCurrentPositionTimer) {
      clearTimeout(accessory.pollCurrentPositionTimer)
      accessory.pollCurrentPositionTimer = null
    }
    ...
    accessory.log.info("Polling current position: " + position)

    accessory.pollCurrentPositionTimer = setTimeout(async () => {
      accessory.pollCurrentPositionTimer = null

      await accessory.am43device.updatePositionAsync()
      position = 100 - accessory.am43device.position // In AM43 Devices 100% means fully closed, but in HomeKit 100% means fully opened
      accessory.log.info("Polled current position: " + position)
      ...
      if (accessory.position !== accessory.targetPosition) {
        this.pollCurrentPosition(accessory, counter + 1)
      }
    }, 1000)
  }
}
