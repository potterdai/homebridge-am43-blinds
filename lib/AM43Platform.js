...
class AM43Platform {
  ...
  configurePropertiesOnAccessory(accessory) {
    ...
    accessory.disconnectIfUninteracted = () => {
      ...
      if (
        ...
        accessory.logDebug("Disconnecting AM43 due to HAP inactivity")
        accessory.am43device.disconnectAsync()
      }
    }
    ...
    accessory.scanForMissingDevices = () => {
      ...
      this.logDebug("Started scan for missing devices")
    }
    ...
    accessory.updateInformation = async function () {
      ...
      this.logDebug("Updating device information from poll")
  ...
  configureDeviceOnAccessory(accessory, device) {
    ...
    device.on("direction", (direction) => {
      this.logDebug("Notifying of new direction (0 down, 1 up): " + direction)
    ...
    device.on("targetPosition", (position) => {
      ...
      this.logDebug("Notifying of new target position: " + targetPosition)
      accessory.windowCoveringService
        .getCharacteristic(this.Characteristic.TargetPosition)
    ...
    device.on("position", (position) => {
      ...
      this.logDebug("Notifying of new position: " + position)
    ...
    device.on("batteryPercentage", (percentage) => {
      this.logDebug("Notifying of new battery percentage: " + percentage)
  ...
  configureInformationServiceOnAccessory(accessory) {
    ...
    service
      ...
      .onGet(() => {
        if (!accessory.am43device) {
          accessory.logDebug("No device found please try again", null)
          return
        }
  ...
  configureWindowCoveringServiceOnAccessory(accessory) {
    ...
    service
      ...
      .onGet(async () => {
        ...
        if (
          ...
          accessory.logDebug("Requesting position update", accessory.am43device.isConnected)
        ...
        accessory.logDebug("Reporting position: " + position)
        return position
      })
    ...
    service
      ...
      .onGet(() => {
        ...
        accessory.logDebug("Reporting target position: " + targetPosition)

        return targetPosition
      })
      .onSet(async (value) => {
        if (!accessory.am43device) {
          accessory.logDebug("No device found please try again")
          return
        }
        accessory.lastHAPInteraction = Date.now()

        const targetPosition = 100 - value // In AM43 Devices 100% means fully closed, but in HomeKit 100% means fully opened
        accessory.logDebug("setting target position: " + targetPosition)
        try {
    ...
    service
      ...
      .onGet(() => {
        ...
        accessory.lastHAPInteraction = Date.now()
        accessory.logDebug("Reporting direction: " + accessory.am43device.direction)
