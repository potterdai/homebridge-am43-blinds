...
class AM43Device extends EventEmitter {
  constructor(peripheral) {
    ...
    this.peripheral.on("connect", () => {
      this.debugInfo(`Device connected: ${this.id}`)
      this.isConnected = true
    })
    this.peripheral.on("disconnect", () => {
      this.debugInfo(`Device disconnected: ${this.id}`)
  ...
  async connectAsync() {
    ...
    if (this.discoveringPromise == null && this.blindsControlCharacteristic == null) {
      this.debugInfo("waitingForDiscoveringPromise")
      ...
      this.discoveringPromise = null
      this.debugInfo("setBlindsControlCharacteristic")
  ...
  checkIfStopped() {
    this.debugLog("[checkIfStopped] " + this.positionHistory)
  ...
  async sendCommandAsync(commandID, data) {
    if (!this.isConnected) {
      if (commandID == StaticVariables.AM43_COMMAND_ID_GET_POSITION
          || commandID == StaticVariables.AM43_COMMAND_ID_GET_BATTERYSTATUS
          || commandID == StaticVariables.AM43_COMMAND_ID_GET_LIGHTSENSOR) {
            this.connectAsync()
            return
      }
